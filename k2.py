...
def translate():
    ...
    for element in os.listdir('final/3_ready'):
        if NAME not in element:
            ...
            result_line, not_in_dict = main_file_searching_translations(main_foreign_lang, main_foreign_word, foreign_nouns, result_line, i, not_in_dict)
...
def main_file_searching_translations(language, word, foreign, results_file, i, not_in_dict):
    ...
    for line in f:
        ...
        if len(results_file[noun]) != i:
            if language != LANG:
                ...
                if tr == []:
                    if noun not in not_in_dict:
                        not_in_dict.append(noun)
    ...
    f.close()
    not_translated(not_in_dict)
    return results_file, not_in_dict
